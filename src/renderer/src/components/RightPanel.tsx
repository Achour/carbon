...
function Tab({
  ...
  return (
    ...
    >
      ...
      <span className={cn("max-w-36 truncate", preview && "italic")}>
        {label}
      </span>
...
function PathBar({
  ...
  const fullPath = entry.diff
    ? `${entry.diff.cwd}/${entry.diff.file}`
    : entry.path;
  return (
        ...
        <div className="max-h-72 overflow-y-auto p-1">
          {results.length > 0 ? (
            results.map((r, i) => {
              ...
              const dir = r.rel
                .slice(0, r.rel.length - name.length)
                .replace(/\/$/, "");
              return (
                <button
                  key={r.path}
...
export function RightPanel(): React.JSX.Element | null {
  ...
  const [dockWidth, setDockWidth] = React.useState<number>(() => {
    ...
    return Number.isFinite(saved) && saved >= DOCK_MIN_PX
      ? saved
      : DOCK_DEFAULT_PX;
  });
  ...
  const onDockPointerMove = (e: React.PointerEvent<HTMLDivElement>): void => {
    ...
    setDockWidth(
      Math.max(
        DOCK_MIN_PX,
        Math.min(next, Math.round(rect.width) - VIEWER_RESERVED_PX),
      ),
    );
  };
  ...
  const onHandlePointerMove = (e: React.PointerEvent<HTMLDivElement>): void => {
    ...
    const chatLeft =
      panel?.previousElementSibling?.getBoundingClientRect().left ?? 0;
  ...
  const currentIsChanges =
    typeof current === "string" && current.startsWith("changes:");
  ...
  const currentCanvasId =
    typeof current === "string" && current.startsWith("canvas:")
      ? current.slice(7)
      : null;
  ...
  const activeIsMarkdown =
    !!activeEntry && !activeEntry.diff && MARKDOWN_RE.test(activeEntry.name);
  ...
  // pane's width less the chat column's reservation. Measured off the parent
  ...
  React.useLayoutEffect(() => {
    ...
    const measure = (): void =>
      setAvail(
        Math.max(
          0,
          Math.round(parent.getBoundingClientRect().width - CHAT_RESERVED_PX),
        ),
      );
    measure();
  ...
  const minPx = avail === null ? PANEL_MIN_PX : Math.min(PANEL_MIN_PX, avail);
  const cap =
    avail === null ? `calc(100vw - ${CHAT_RESERVED_PX}px)` : `${avail}px`;
    ...
    >
      ...
      {showContents && (
        ...
        >
          ...
          >
            ...
            <TabScroller>
              ...
              {openFiles.map((file) => (
                <Tab
                  ...
                  onDoubleClick={
                    file.preview ? () => promoteTab(file.path) : undefined
                  }
            ...
            <WithTooltip
              label={panelMaximized ? "Restore panel" : "Maximize panel"}
            >
              <Button
                size="icon-sm"
                variant="ghost"
          ...
          {showBreadcrumb && (
            <div className="flex h-8 shrink-0 items-center gap-2 border-b border-border/60 pr-1.5 pl-3">
              <div className="min-w-0 flex-1">
                {activeEntry && (
                  <PathBar entry={activeEntry} cwd={selectedCwd} />
                )}
              </div>
              {activeIsMarkdown && (
              ...
              <WithTooltip
                label={dockOpen ? "Hide file tree" : "Show file tree"}
              >
                <Button
                  size="icon-sm"
                  variant="ghost"
            ...
            {dockOpen &&
              ...
              !currentIsPreview && (
                <div
                  data-right-dock
                  style={{
                    width: `min(${dockWidth}px, calc(100% - ${VIEWER_RESERVED_PX}px))`,
                  }}
      ...
      >
        <DialogContent>
          <DialogTitle>
            Save changes to “
            {openFiles.find((f) => f.path === confirmClose)?.name}”?
          </DialogTitle>
          <DialogDescription>
            This file has edits that have not been written to disk. Closing the
            tab discards them.
          </DialogDescription>

 ...
 * state is store state, and the one surface a delete starts from — the thread
 * header's `+` popover — closes the instant the ✕ is clicked, which would take the question
 * off screen with the answer still pending.
 *
 * It asks at all because a side chat is no longer temporary. While the app
 * deleted them on quit, the ✕ in the closed list was discarding something the
    ...
    <Dialog open={pending !== null} onOpenChange={(open) => !open && confirmSideChatDelete(null)}>
      <DialogContent>
        ...
        <DialogDescription>
          ...
          everything in it is deleted permanently. It is stored by Carbon, so there is no file to
          recover and nothing in the Trash.
        </DialogDescription>

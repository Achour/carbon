...
Gotchas the code exists to handle: subagent turns live in separate `<session>/subagents/agent-*.jsonl` files (walk 6 levels or lose ~16% of spend); one Claude API response is logged once per content block, so dedupe on `message.id` + `requestId`; Codex's `token_count` events carry a running total *and* a per-call delta and no model, so `CodexFileReader` carries the model forward and sums only the delta; Codex's `input_tokens` is inclusive of `cached_input_tokens`, Claude's is not. Rates are fetched from LiteLLM's public `model_prices_and_context_window.json` (cached 24h in `userData/usage-rates.json`, fallback = the built-in table in `usageScan.ts`) — a static table can't price the Codex-only slugs, which bill ~4× the GPT-5 family they're named after. Claude's 1-hour cache writes (2× input) and fast mode are honored; unpriced tokens are reported, never treated as free. Cells store **tokens, not money** — cost is applied at read time, so a rate refresh reprices without re-reading a byte. Per-file results are cached under `(path, mtime, size)` in `userData/usage-cache.json` — bump `CACHE_VERSION` whenever *parsing* changes.

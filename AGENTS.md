...
Gotchas the code exists to handle: subagent turns live in separate `<session>/subagents/agent-*.jsonl` files (walk 6 levels or lose ~16% of spend); one Claude API response is logged once per content block, so dedupe on `message.id` + `requestId`; Codex's `token_count` events carry a running total *and* a per-call delta and no model, so `CodexFileReader` carries the model forward and sums only the delta; Codex's `input_tokens` is inclusive of `cached_input_tokens`, Claude's is not. Pricing is a static list-price table (`rateFor`, longest-prefix match) that honors Claude's 5m/1h cache-write split and fast mode; unpriced tokens are reported, never treated as free. Per-file results are cached under `(path, mtime, size)` in `userData/usage-cache.json` — bump `CACHE_VERSION` whenever parsing or pricing changes, since the cache stores derived cells.
